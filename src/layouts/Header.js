...
export default function Header({ onSkin }) {
  ...
  // Ensure sidebar skin is set to dark by default
  useEffect(() => {
    const savedSidebarSkin = localStorage.getItem("sidebar-skin");
    if (!savedSidebarSkin) {
      localStorage.setItem("sidebar-skin", "dark");
      document.querySelector("html").setAttribute("data-sidebar", "dark");
    }
  }, []);

  // Fetch the current user from Firebase and user details from Firestore
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (user) {
        setUser(user); // Set Firebase user data
        const userDocRef = doc(db, "Users", user.uid); // Get the user's document from Firestore
        const userDoc = await getDoc(userDocRef);

        if (userDoc.exists()) {
          const userData = userDoc.data();
          setFullName(userData.fullName); // Set the full name from Firestore
        }
      } else {
        setUser(null); // Set null if no user is logged in
        setFullName(""); // Clear full name if no user is logged in
      }
    });
      ...
      <Dropdown className="dropdown-profile ms-3 ms-xl-4" align="end">
        ...
        <Dropdown.Menu className="mt-10-f">
          <div className="dropdown-menu-body">
            ...
            {!user && <p className="fs-sm text-secondary">Not logged in</p>}
